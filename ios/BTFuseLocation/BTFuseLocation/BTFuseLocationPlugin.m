...
#import <BTFuseLocation/BTFuseLocationPlugin.h>
#import <BTFuseLocation/BTFuseLocationClient.h>
#import <BTFuseLocation/BTFuseLocationAccuracy.h>
#import <BTFuseLocation/BTFuseLocationEvent.h>

@implementation BTFuseLocationPlugin {
    NSMutableDictionary<NSString*, BTFuseLocationClient*>* $clients;
    NSString* $callbackID;
}
...
- (void) initHandles {
    __weak BTFuseLocationPlugin* weakSelf = self;
    
    [self attachHandler:@"/assertSettings" callback:^(BTFuseAPIPacket* packet, BTFuseAPIResponse* response) {
        BTFuseLocationPlugin* strongSelf = weakSelf;
        
        NSError* error = nil;
        NSDictionary* data = [packet readAsJSONObject: error];
        
        if (error != nil) {
            [response sendError:[[BTFuseError alloc] init:[strongSelf getID] withCode: 0 withError:error]];
            return;
        }
        
        NSString* clientID = [data objectForKey:@"subscriptionID"];
        if (clientID == nil) {
            [response sendError:[[BTFuseError alloc] init:[strongSelf getID] withCode: 0 withMessage:@"Subscription ID required"]];
            return;
        }
        
        BTFuseLocationClient* client = nil;
        ...
        if (client == nil) {
            [response sendError:[
                [BTFuseError alloc] init:[strongSelf getID] withCode: 0 withMessage: [
    ...
    [self attachHandler:@"/callback" callback:^(BTFuseAPIPacket* packet, BTFuseAPIResponse* response) {
        BTFuseLocationPlugin* strongSelf = weakSelf;
...
//        NSNumber* interval = [data objectForKey:@"interval"];
        ...
        BTFuseLocationClient* client = [[BTFuseLocationClient alloc] init:[strongSelf getContext] withDelegate:strongSelf];
        ...
        CLLocationAccuracy locAccuracy = ([desiredAccuracy intValue] == BTFuseLocationAccuracyFine) ? kCLLocationAccuracyBest : kCLLocationAccuracyThreeKilometers;
    ...
    [self attachHandler:@"/unsubscribe" callback:^(BTFuseAPIPacket* packet, BTFuseAPIResponse* response) {
        BTFuseLocationPlugin* strongSelf = weakSelf;
...
- (void) locationManager:(CLLocationManager*) lm didUpdateLocations:(NSArray<CLLocation*>*) locations {
    ...
    NSString* payload = [BTFuseJSONSerializer serialize:event withError:&serializationError];
